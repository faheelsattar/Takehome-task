...
contract OpynPerpVault is ERC20, ReentrancyGuard, Ownable {
    ...
    ICurve public immutable curvePool; //$GAS: should be immutable inorder to prevent an SLOAD as it isnt being updated

    /// @dev ecrv lp token that is received from depositng into the curve pool
    IERC20 immutable ecrv; //$GAS: should be immutable

    /// @dev stake dao sdecrvAddress
    address public immutable sdecrvAddress;
    ...
    event CapUpdated(uint80 newCap);
    ...
    constructor(
        ...
        sdecrv = IStakeDao(_sdecrvAddress);
        ecrv = sdecrv.token();
     ...
     */
    function setCap(uint80 _newCap) external onlyOwner {
        cap = _newCap;
    ...
    function totalStakedaoAsset() public view returns (uint256) {
        uint256 debt = 0;
        address[] memory vaults = actions; //$GAS: actions should be stored in memory to prevent sload in each iter

        for (uint256 i = 0; i < vaults.length; i++) {
            debt = debt.add(IAction(vaults[i]).currentValue());
        }
    ...
    function depositETH(uint256 minEcrv) external payable nonReentrant {
        ...
        ecrv.safeIncreaseAllowance(sdecrvAddress, ecrvToDeposit); //would be better to set max approval before deposit via ui
        ...
        uint256 sdecrvDeposited = totalWithDepositedAmount -
            totalSdecrvBalanceBeforeDeposit; //@Gas: can be replace with normal subtract as it wont overflow
    ...
    function withdrawETH(uint256 _share, uint256 minEth) external nonReentrant {
        ...
        uint256 ethOwedToUser = ethReceived - fee; //wont overflow as fee is always going to be less then ethRecieved
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _allocationPercentages.length; i = i + 1) {
            ...
            uint256 newAmount = totalAsset.mul(_allocationPercentages[i]).div(
                BASE
            );
     ...
     */
    function setWithdrawalFeePercentage(uint32 _newWithdrawalFeePercentage)
        external
        onlyOwner
    {
     ...
     * @dev set the percentage that should be reserved in vault for withdraw
     */
    function setWithdrawReserve(uint32 _reserve) external onlyOwner {
